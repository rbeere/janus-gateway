...
typedef struct janus_sip_session {
	...
	janus_mutex mutex;
	OpusEncoder *opus_encoder;	/* Opus encoder instance to use for webrtc connection */
	OpusDecoder *opus_decoder;		/* Opus decoder instance */
	FILE *recording;
...
#define JANUS_SIP_ERROR_LIBOPUS_ERROR       453
		...
		if(video) {
			...
			if(session->media.has_audio && session->media.audio_rtp_fd) {
				...
				if(session->media.has_srtp_local) {
					...
					// open wav if not done yet
					/*
					if(session->recording == NULL) {
						char filename[255];
						g_snprintf(filename, 255, "/tmp/janusrecord.wav");
						session->recording = fopen(filename, "wb");
						if(session->recording == NULL) {
							...
							wav_header header = {
								...
								1,
								1,
								8000, // audiobridge->sampling_rate,
								16000, // audiobridge->sampling_rate * 2,
								2,
								16,
								{'d', 'a', 't', 'a'},
								0
							};
							if(fwrite(&header, 1, sizeof(header), session->recording) != sizeof(header)) {
								JANUS_LOG(LOG_ERR, "Error writing WAV header...\n");
							}
							fflush(session->recording);
							session->record_lastupdate = janus_get_monotonic_time();
						}
					}					
					 */
					...
					guint32 timestamp = ntohl(header->timestamp);
					guint16 seq = ntohs(header->seq_number);
					JANUS_LOG(LOG_VERB, "got webrtc audio data , rtp len : %"SCNu16" , ts :%"SCNu32" , seq : %"SCNu16" \n", len, timestamp, seq);		
					int i = 0;
					unsigned char * opus_data = (unsigned char*) buf ;
					opus_int16 * rawData = g_malloc0(BUFFER_SAMPLES*sizeof(opus_int16));
					int rawLen = opus_decode(session->opus_decoder , opus_data + 12 , len - 12 , rawData, BUFFER_SAMPLES, USE_FEC);
					JANUS_LOG(LOG_VERB, "opus decode returned : %"SCNu16"\n", rawLen);

					// RECORDING stuff
					/*
					if(session->recording != NULL ) {
						fwrite(rawData, sizeof(opus_int16), rawLen, session->recording);
						// Every 5 seconds we update the wav header 
						gint64 now = janus_get_monotonic_time();
						if(now - session->record_lastupdate >= 5*G_USEC_PER_SEC) {
							session->record_lastupdate = now;
							// Update the length in the header 
							fseek(session->recording, 0, SEEK_END);
							long int size = ftell(session->recording);
							if(size >= 8) {
								size -= 8;
								fseek(session->recording, 4, SEEK_SET);
								fwrite(&size, sizeof(uint32_t), 1, session->recording);
								size += 8;
								fseek(session->recording, 40, SEEK_SET);
								fwrite(&size, sizeof(uint32_t), 1, session->recording);
								fflush(session->recording);
								fseek(session->recording, 0, SEEK_END);
							}
					...
					*/
					...
					// char * alawData = g_malloc0(rawLen);
					for ( i=0 ; i<rawLen ; i++) {
						buf[i+12] = Snack_Lin2Alaw( rawData [i]);
					}
					free(rawData);
					header->type = 8;
					header->timestamp = htonl(timestamp / 6);
					timestamp = ntohl(header->timestamp);
					seq = ntohs(header->seq_number);					
					guint16 ptype = header->type;
					JANUS_LOG(LOG_VERB, "webrtc audio forward to sip , rtp len : %"SCNu16" , ts :%"SCNu32" , seq : %"SCNu16" , pt : %"SCNu16" \n", rawLen + 12, timestamp, seq, ptype);				
					send(session->media.audio_rtp_fd, buf, rawLen + 12, 0);
				}
			}
		}
	}
...
static void *janus_sip_handler(void *data) {
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(!strcasecmp(request_text, "register")) {
			...
			g_atomic_int_set(&session->hangingup, 0);
			// insert opus decoder / encoder
			int error = 0;
			session->opus_encoder = opus_encoder_create(8000, 1, OPUS_APPLICATION_VOIP, &error);
			if(error != OPUS_OK) {
				JANUS_LOG(LOG_ERR, "Error creating Opus encoder for SIP plugin \n");
				error_code = JANUS_SIP_ERROR_LIBOPUS_ERROR;
				goto error;
			}
			error = 0;
			session->opus_decoder = opus_decoder_create(8000, 1, &error);
			if(error != OPUS_OK) {
				JANUS_LOG(LOG_ERR, "Error creating Opus decoder for SIP plugin \n");
				error_code = JANUS_SIP_ERROR_LIBOPUS_ERROR;
				goto error;
			}
...
void janus_sip_sofia_callback(nua_event_t event, int status, char const *phrase, nua_t *nua, nua_magic_t *magic, nua_handle_t *nh, nua_hmagic_t *hmagic, sip_t const *sip, tagi_t tags[])
{
	...
	/* SIP responses */
		...
		case nua_r_invite: {
			...
			/* Send SDP to the browser */
			json_t *jsep = json_pack("{ssss}", "type", "answer", "sdp", janus_sdp_write(sdp) /*fixed_sdp*/ );
			json_t *call = json_object();
			json_object_set_new(call, "sip", json_string("event"));
			json_t *calling = json_object();
...
char *janus_sip_sdp_manipulate(janus_sip_session *session, janus_sdp *sdp, gboolean answer) {
	...
	janus_sdp_remove_payload_type( sdp , 103);
	janus_sdp_remove_payload_type( sdp , 104);
	janus_sdp_remove_payload_type( sdp , 111);
	janus_sdp_remove_payload_type( sdp , 9);
	janus_sdp_remove_payload_type( sdp , 13);
	janus_sdp_remove_payload_type( sdp , 106);
	janus_sdp_remove_payload_type( sdp , 105);
	janus_sdp_remove_payload_type( sdp , 96); 
	janus_sdp_remove_payload_type( sdp , 0); 
	...
	while(temp) {
		...
		if(m->type == JANUS_SDP_AUDIO) {
			...
			m->ptypes = g_list_append(m->ptypes, GINT_TO_POINTER(8));
			m->attributes = g_list_append(m->attributes, janus_sdp_attribute_create("rtpmap" ,  "8 PCMA/8000"));			
			if(session->media.has_srtp_local) {
				char *crypto = NULL;
...
static void *janus_sip_relay_thread(void *data) {
	...
	int samples = 160;
	char buffer[BUFFER_SAMPLES];
	memset(buffer, 0, BUFFER_SAMPLES);
	opus_int16 encoded[BUFFER_SAMPLES];
	memset(encoded, 0, BUFFER_SAMPLES * sizeof(opus_int16));	
	/* Loop */
	int num = 0;
	gboolean goon = TRUE;
	int astep = 0, vstep = 0;
	...
	while(goon && session != NULL && !session->destroyed &&
		...
		for(i=0; i<num; i++) {
			if(fds[i].revents & ( /*POLLERR |*/ POLLHUP)) {
				...
				JANUS_LOG(LOG_ERR, "[SIP-%s]   -- fd : %d %d (%s)\n", session->account.username, i , errno, strerror(errno));
				if(session->media.updated)
					break;
				...
				if(session->media.audio_rtp_fd != -1 && fds[i].fd == session->media.audio_rtp_fd) {
					...
					bytes = recvfrom(session->media.audio_rtp_fd, buffer, BUFFER_SAMPLES, 0, (struct sockaddr*)&remote, &addrlen);
					...
					guint32 timestamp = ntohl(header->timestamp);
					guint16 seq = ntohs(header->seq_number);
					JANUS_LOG(LOG_VERB, "Relay packet Audio unmodified : SSRC=%"SCNu32" , seq=%"SCNu16" , ts=%"SCNu32" \n",  ssrc , seq , timestamp );
					...
					session->media.context.a_base_ts++;
					/* Compute a coherent timestamp and sequence number */
					//session->media.context.a_last_ts = (timestamp-session->media.context.a_base_ts) + session->media.context.a_base_ts_prev+(astep ? astep : 960);	/* FIXME */
					session->media.context.a_last_ts = session->media.context.a_base_ts_prev + ( session->media.context.a_base_ts * 960 ) ; // (astep ? astep : 960);	/* FIXME */
					
					guint16 newSeq = (seq-session->media.context.a_base_seq)+session->media.context.a_base_seq_prev+1;
					if ( newSeq <= session->media.context.a_last_seq ) {
						JANUS_LOG(LOG_WARN, "Detected timestamp moving backward , setting new base." );
						session->media.context.a_base_seq_prev = session->media.context.a_last_seq;
						session->media.context.a_base_seq = seq;
					}
					//session->media.context.a_last_seq = (seq-session->media.context.a_base_seq)+session->media.context.a_base_seq_prev+1;
					session->media.context.a_last_seq = session->media.context.a_last_seq + 1 ; 
					...
					header->type = 96;
					JANUS_LOG(LOG_VERB, "Relay from sip received bytes : %"SCNu16" \n", bytes );
					gint alawLen = bytes - 12;
					// opus_int16 *outBuffer = g_malloc0( ( BUFFER_SAMPLES*sizeof(opus_int16) ) + 12 );
					// memcpy( outBuffer , buffer , 12);
					int rawLen = 0;
					int i = 0;
					for ( i=0; i<alawLen ; i++ ) {
						encoded[rawLen++] = Snack_Alaw2Lin( buffer[i+12] );
						/*
						encoded[rawLen++] = ( lin >> 8 ) & 0xff;
						encoded[rawLen++] = lin & 0xff; */
					}
					JANUS_LOG(LOG_VERB, "Relay from sip alawlen : %"SCNu16" \n", alawLen );
					int opusLen = opus_encode(session->opus_encoder, encoded, rawLen, buffer+12, BUFFER_SAMPLES-12);
					JANUS_LOG(LOG_VERB, "opus returned: %"SCNu16" \n", opusLen );
					// header->length = opusLen;				
					if(ats == 0) {
						ats = timestamp;
					} else if(astep == 0) {
						astep = timestamp-ats;
